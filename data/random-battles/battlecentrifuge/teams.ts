...
export class BattleCentrifugeTeams {
	...
	loadSavedSets(userid: string): RandomTeamsTypes.RandomSet[] | null {
		const fs = require('fs');
		...
		try {
			...
			return setStrings.map((str: string) => JSON.parse(str));
		} catch (error) {
			return null;
		}
	}
	...
	randomTeam() {
		...
		// Check for saved sets in Battle Centrifuge Plus
		if (this.format.id === 'gen9battlecentrifugeplus') {
			const userid = (this.format as any).requestedTeam;
			if (userid) {
				const savedSets = this.loadSavedSets(userid);
				if (savedSets) {
...
export default BattleCentrifugeTeams;
